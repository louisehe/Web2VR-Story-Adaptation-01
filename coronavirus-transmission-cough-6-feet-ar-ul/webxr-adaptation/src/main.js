 ...
 * (NYT, April 14 2020). Same engine family as Story 5: the explainer is a GLB
 * (transmission-200414-01.glb) with an animated camera, a "Take 001" clip (~25.8 s), distance
 * markers (3/6/10/16/26 ft), figures, and an opacity-driver system (opacity_driver_N.x × 100 →
 * opacity_target_N). 13 beats scrub the clip to a normalized position and show the caption.
 *
 * Note: the cough DROPLET cloud in the original is a separate .pcd point cloud (not in the GLB);
 * if captured it can be added as THREE.Points driven by the same clip.
 */
...
const fill = new THREE.DirectionalLight(0xcfe0ff, 0.5); fill.position.set(-4, 2, -3); scene.add(fill);
