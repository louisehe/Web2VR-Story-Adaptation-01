/**
 * WebXR adaptation — "Les masques, ça marche." (NYT, Nov 11 2020, fr).
 * The whole explainer is one GLB (masks-088.glb): an animated scene ("Take 001", 93 s) with an
 * animated camera, baked particle motion (brownian / impact / electrostatic), mask & fibre geometry,
 * and an opacity-driver system (each opacity_driver_N node's local x, ×100, is the opacity of the
 * matching opacity_target_N mesh — that's how the original fades parts in and out).
 *
 * The original scroll-scrubs the timeline. Here, 23 beats each scrub the clip to a normalized
 * position (from the captured config) and show the caption; the GLB's own camera frames each beat.
 */
...
const DURATION = data.duration || 93.04;
...
const fill = new THREE.DirectionalLight(0xbfd4ff, 0.6); fill.position.set(-4, -2, -3); scene.add(fill);

// VR rig (headset follows the GLB camera pose per beat)
const rig = new THREE.Group(); const xrCam = new THREE.PerspectiveCamera(60, innerWidth / innerHeight, 0.01, 1000); rig.add(xrCam); scene.add(rig);
...
if (!cam) { cam = new THREE.PerspectiveCamera(40, innerWidth / innerHeight, 0.01, 1000); cam.position.set(0, 0, 6); }
cam.near = 0.01; cam.far = 2000; cam.updateProjectionMatrix();

// opacity driver/target system
