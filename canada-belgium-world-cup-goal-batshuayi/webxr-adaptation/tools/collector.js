/**
 * NYT story asset collector — Belgium's Long-Ball Goal Sinks a Determined Canada
 * https://www.nytimes.com/interactive/2022/11/23/sports/world-cup/canada-belgium-world-cup-goal-batshuayi.html
 *
 * 用法：
 *  1. Chrome 登录后打开上面的故事页面，手动滚到底再滚回顶部（确保 3D 资产全部加载）。
 *  2. F12 → Console（若禁止粘贴，先输入 allow pasting 回车），粘贴本脚本全文回车。
 *  3. 允许“下载多个文件”。文件以 nyt_belgium__ 前缀存入下载目录。
 *  4. 把 nyt_belgium__* 文件按类型复制到 ../captures/active/ 下：
 *       *model*.glb/.bin       → captures/active/models/
 *       *texture*              → captures/active/textures/
 *       *data*.json            → captures/active/data/
 *       story_text/manifest/page.html → captures/active/metadata/
 *
 * 仅收集页面公开加载的资产，不绕过付费墙/鉴权/DRM。
 */
(async () => {
  const PREFIX = 'nyt_belgium__';
  ...
  const STORY = /newsgraphics|nytg|belgium|canada|goal|batshuayi|bel|can|world-cup/i;
  const classify = u => {
  ...
  document.querySelectorAll('script:not([src])').forEach(s => {
    ...
    if (/WEBGL_DATA|"slides"|"steps"|"camera"|"frameNo"|sheetsById|tracksByObject|\.glb/i.test(t) && t.length < 2e6) structure.push(t.slice(0, 500000));
  });
