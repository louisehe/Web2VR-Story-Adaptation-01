/**
 * WebXR adaptation — "Belgium's Long-Ball Goal Sinks a Determined Canada" (NYT, Nov 23 2022)
 * Same NYT 3-D goal template as spain-germany, with a generalized opacity/annotation engine.
 * Rebuilt from captured public assets:
 *  - world-cup-2022-belgium-canada-7.glb  (players, ball, trajectory arc, projection plane, projection camera)
 *  - BELCAN_FieldLines.glb                (field lines, goal, flags, arrow, open space, penalty point)
 *  - bfNNNN_*.webp frame sequence         (photo scrub)
 *  - data_state.json                      (Theatre.js timeline: camera path, frame numbers, opacity tracks)
 */
...
const FREEZE_FRAME = '18';
const MAIN_GLB = 'nyt_belgium__model__assets_world-cup-2022-belgium-canada-7.glb';
const ANNO_GLB = 'nyt_belgium__model__assets_BELCAN_FieldLines.glb';
...
// camera target is a static override here (no track)
const tgtStatic = (anim.static['Camera Target'] || {}).position || { x: 0, y: 0.05, z: 0 };
const camStatic = anim.static['Camera'] || {};
const FOV = camStatic.fov || 15;
const camPosAt = (t) => new THREE.Vector3(camX(t), camY(t), camZ(t));
const camTgtAt = () => new THREE.Vector3(tgtStatic.x, Math.max(tgtStatic.y, 0.1), tgtStatic.z);
...
function classifyMesh(o) {
  ...
  if (/player_mesh_/.test(nm)) {
    ...
    let base = 0x9aa0a6;
    if (/goalkeeper/.test(nm)) base = parseInt(TEAM.goalkeeper.slice(1), 16);
    else if (/belgium/.test(nm)) base = parseInt(TEAM.belgium.slice(1), 16);
    else if (/canada/.test(nm)) base = parseInt(TEAM.canada.slice(1), 16);
  ...
  if (/soccerball|ball_texture/.test(nm)) { o.material = new THREE.MeshLambertMaterial({ color: 0xffffff }); return; }
...
function applyTime(t) {
  ...
  if (!presenting) { camera.position.copy(camPosAt(t)); camera.lookAt(camTgtAt()); }
}
function placeRigAt(t) {
  const pos = camPosAt(t), tgt = camTgtAt();
...
function goToBeat(i, instant = false) {
  ...
  if (renderer.xr.isPresenting) { fadeSphere.visible = true; tween = { kind: 'xrfade', start: performance.now(), dur: 700, targetT: beat.time, applied: false }; }
  else {
...
renderer.xr.addEventListener('sessionstart', () => { placeRigAt(BEATS[beatIndex].time); timelineT = BEATS[beatIndex].time; applyTime(timelineT); });
...
renderer.setAnimationLoop(() => {
  ...
  if (tween) {
    ...
    if (tween.kind === 'scrub' || tween.kind === 'ease') { const s = u * u * (3 - 2 * u); timelineT = tween.fromT + (tween.targetT - tween.fromT) * s; applyTime(timelineT); if (u >= 1) tween = null; }
