/**
 * NYT story asset collector — Weston McKennie Is Going to Want This One Back
 * https://www.nytimes.com/interactive/2022/11/25/sports/world-cup/usa-weston-mckennie-england.html
 *
 * 用法：
 *  1. Chrome 登录后打开上面的故事页面，手动滚到底再滚回顶部（确保 3D 资产全部加载）。
 *  2. F12 → Console（若禁止粘贴，先输入 allow pasting 回车），粘贴本脚本全文回车。
 *  3. 允许“下载多个文件”。文件以 nyt_mckennie__ 前缀存入下载目录。
 *  4. 把 nyt_mckennie__* 文件按类型复制到 ../captures/active/ 下：
...
(async () => {
  ...
  const STORY = /newsgraphics|nytg|mckennie|usa|england|usmnt|eng|goal|world-cup/i;
  const classify = u => {
